...
> [!TIP]
> As a PSE, you'll spend most of your day working with APIs — integrating Eightfold with customer HRIS/ATS systems, debugging failed syncs, and building custom workflows. This is probably the MOST important deliverable for your interview.
...
> **Answer:** "401 means authentication failed. I'd check: (1) Is the API key/token present in the Authorization header? (2) Has it expired? (3) Is it being sent in the correct format (Bearer vs Basic)? (4) Was the key recently rotated on the customer side without updating Eightfold's config?"
...
Host: api.eightfold.ai                 ← Required header
...
## Part C: Eightfold-Specific Integration Patterns

### How Eightfold Integrates with Enterprise Systems

```
┌─────────────────┐     API      ┌──────────────────┐     API      ┌────────────┐
│  Customer ATS   │ ──────────→  │   Eightfold AI   │ ──────────→  │  Customer  │
...
**What Eightfold does in this architecture:**
...
| Webhooks | Real-time event notifications | "Notify Eightfold when a new job is posted" |
| RPA (Robotic Process Automation) | Legacy systems with NO API | Eightfold's RPA bot logs into the system and extracts data |

### Common PSE Integration Tasks

1. **Configure data mapping** — Map fields between Eightfold and customer system
   - Customer's "First Name" + "Last Name" → Eightfold's "Full Name"
   - Customer's "Job Code" → Eightfold's "Requisition ID"
...
5. Compare record counts: How many in Workday vs how many in Eightfold?
...
2. Check if Eightfold's API supports custom fields / extended attributes
